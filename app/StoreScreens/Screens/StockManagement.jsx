...
const StockItem = ({ item, onDeleteProduct }) => {
 ...
 const handleSave = async () => {
   try {
     setIsSaving(true);
     await axios.put(`http://192.168.232.249:3500/citystore/Updatemenu/${item._id}`, {
   ...
   Alert.alert(
     ...
     [
       { text: "Cancel", style: "cancel" },
       {
         ...
         onPress: async () => {
           try {
             await axios.delete(`http://192.168.232.249:3500/citystore/Deletemenu/${item._id}`);
           } catch (error) {
...
export default function StockManagement({ navigation }) {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      try {
        ...
        const response = await axios.get(`http://192.168.232.249:3500/citystore/getallmenu?storeId=${storeId}`);
  ...
  const handleAddItem = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.232.249:3500/citystore/Addmenu', formData, {
        headers: {
      ...
      if (response.status === 201) {
        ...
        const menuResponse = await axios.get(`http://192.168.232.249:3500/citystore/getallmenu?storeId=${storeId}`);
